...
def _build_system_prompt(context: Optional[dict] = None) -> str:
    ...
    system_parts = [
        ...
        "User: 'Where is there a 30% chance of temperature above 35°C in 2060?' -> switch_to_ensemble_mode(models=['ACCESS-CM2','CanESM5','CESM2','CMCC-CM2-SR5','EC-Earth3','GFDL-ESM4','INM-CM5-0','IPSL-CM6A-LR','MIROC6','MPI-ESM1-2-HR','MRI-ESM2-0'], scenarios=['ssp585'], date='2060-01-01', variable='tas', unit='Celsius (°C)', masks=[{id:1, kind:'probability', variable:'tas', unit:'Celsius (°C)', lowerBound:35, upperBound:1000000, probabilityThreshold:0.3}]).",
        "User: 'Find areas where potatoes can grow in 30 years, worst-case, 20% probability, temperature and precipitation' -> switch_to_ensemble_mode(models=['ACCESS-CM2','CanESM5','CESM2','CMCC-CM2-SR5','EC-Earth3','GFDL-ESM4','INM-CM5-0','IPSL-CM6A-LR','MIROC6','MPI-ESM1-2-HR','MRI-ESM2-0'], scenarios=['ssp585'], date='2055-07-01', variable='tas', unit='°C', masks=[{id:1, kind:'probability', variable:'tas', unit:'°C', lowerBound:10, upperBound:25, probabilityThreshold:0.2}, {id:2, kind:'probability', variable:'pr', unit:'mm/day', lowerBound:1.4, upperBound:6.0, probabilityThreshold:0.2}]). NOTE: for precipitation use 'mm/day' (typical range 0–20 mm/day), NOT 'kg m⁻² s⁻¹'.",
